    ...
    for i in range(0, len(tokens), batch_size):
        ...
        try:
            ...
            if agent.requires_scene:
                ...
                # TODO: Implement batch scene processing
                trajectories = []
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    logger.info(f"Processing {len(tokens_to_evaluate_stage_one)} stage one scenarios in batches in thread_id={thread_id}, node_id={node_id}")
    
    # Use batch processing for stage one
    batch_results_stage_one = process_tokens_batch(
        tokens=tokens_to_evaluate_stage_one,
        agent=agent,
    ...
    pdm_results.extend(batch_results_stage_one)
    ...
    logger.info(f"Processing {len(tokens_to_evaluate_stage_two)} stage two scenarios in batches in thread_id={thread_id}, node_id={node_id}")
    
    # Use batch processing for stage two
    batch_results_stage_two = process_tokens_batch(
        tokens=tokens_to_evaluate_stage_two,
        agent=agent,
        scene_loader=scene_loader,
        metric_cache_loader=metric_cache_loader,
        simulator=simulator,
    ...
    pdm_results.extend(batch_results_stage_two)
